...
def input_pipeline_multiple(fnames, batch_size, num_epochs, read_threads):
	...
	patch_list = [read_a_file(fname_queue) for _ in range(read_threads)]
...
batch_pipeline = input_pipeline_multiple(
	file_names, batch_size=50, num_epochs=1000, read_threads=2)
...
ae = rgcautoenc(dimensions=[768, 556], lr=learning_rate, k=1e-00)
...
try:
	while not coord.should_stop():
		...
		if epochs % 50 == 0:
			...
			print(epochs, sess.run(ae['cost'], feed_dict={ae['x']: [subpatch] }))
